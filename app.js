...
const flow=[
  ...
  {key:'contact',question:'Último paso: escribe tu usuario de Instagram, WhatsApp o correo para identificar tu solicitud. Durante esta etapa de pruebas, este resumen se enviará al correo operativo de Raíz Noble. No compartas datos médicos sensibles aquí.',options:[]}
];
...
async function captureLead(){const fingerprint=JSON.stringify(state.profile);if(fingerprint===lastCapturedLead)return;lastCapturedLead=fingerprint;try{await fetch('/api/lead',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(state.profile)})}catch{}}
...
function finishQualification(){captureLead();addMessage(`Gracias, ${state.profile.name||'listo'}. Por lo que me compartiste, el siguiente paso sugerido es: ${recommendation()}.

Preparé un resumen para que el equipo entienda tu objetivo sin hacerte repetir todo.`);const box=document.createElement('div');box.className='summary';const title=document.createElement('strong');title.textContent='RESUMEN DE TU SOLICITUD';const pre=document.createElement('pre');pre.textContent=leadSummary();const actions=document.createElement('div');actions.className='summary-actions';const copy=document.createElement('button');copy.type='button';copy.className='primary';copy.textContent='Copiar resumen';copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(leadSummary());showToast('Resumen copiado')}catch{showToast('Selecciona y copia el resumen manualmente')}});const instagram=document.createElement('a');instagram.href='https://www.instagram.com/andrsaburto/';instagram.target='_blank';instagram.rel='noopener noreferrer';instagram.textContent='Abrir Instagram ↗';actions.append(copy,instagram);box.append(title,pre,actions);chatLog.appendChild(box);chatLog.scrollTop=chatLog.scrollHeight;setReplies([]);const again=document.createElement('button');again.type='button';again.textContent='Hacer otra consulta';again.addEventListener('click',resetChat);quickReplies.appendChild(again);chatInput.placeholder='También puedes hacer una pregunta libre…'}

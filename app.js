...
const serviceDescriptions={
  'Coaching 1 a 1':'acompañamiento personalizado en modalidad online por $2,830 MXN o presencial por $10,440 MXN',
  'Bodybuilding Training System':'sistema online de entrenamiento por $3,140 MXN',
  'Posing Coaching | Aburto Team':'coaching de posing por categoría por $2,620 MXN',
...
function answer(value){const clean=String(value).trim().slice(0,500);if(!clean)return;addMessage(clean,'user');setReplies([]);if(state.profile.interest==='Productos digitales'&&state.step===1&&productDescriptions[clean]){state.profile.interest=clean;addMessage(`Perfecto. ${clean} es ${productDescriptions[clean]}.`);askCurrent();return}const current=flow[state.step];state.profile[current.key]=clean;state.step+=1;if(current.key==='interest'&&clean==='Productos digitales'){addMessage('“5 Claves Antes de Competir” está disponible por $320 MXN. Posing Intensivo y Diario de Progreso se lanzan próximamente.');setReplies(Object.keys(productDescriptions));return}if(current.key==='interest'&&serviceDescriptions[clean])addMessage(`Perfecto. ${clean} es ${serviceDescriptions[clean]}.`);askCurrent()}
function recommendation(){const p=state.profile,target=`${p.interest} ${p.experience} ${p.modality}`.toLowerCase();if(target.includes('coaching 1 a 1'))return p.modality.toLowerCase().includes('presencial')?'Coaching 1 a 1 presencial — $10,440 MXN':p.modality.toLowerCase().includes('en línea')||p.modality.toLowerCase().includes('online')?'Coaching 1 a 1 online — $2,830 MXN':'Coaching 1 a 1 — online $2,830 MXN / presencial $10,440 MXN';if(target.includes('bodybuilding'))return'Bodybuilding Training System — $3,140 MXN';if(target.includes('posing coaching'))return'Posing Coaching | Aburto Team — $2,620 MXN';if(target.includes('preparación para competencia'))return'Preparación para Competencia — $4,180 MXN';if(target.includes('5 claves'))return'5 Claves Antes de Competir — $320 MXN';if(target.includes('posing intensivo'))return'Posing Intensivo — próximamente';if(target.includes('diario de progreso'))return'Diario de Progreso — próximamente';return'Conversación inicial para definir la mejor ruta'}

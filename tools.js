...
root.innerHTML = `
<div class="tool-nav" aria-label="Herramientas"><button data-tool="macros" class="btn">Calculadora de macros</button><button data-tool="checklist" class="btn alt">Checklist de competencia</button><button data-tool="timer" class="btn alt">Temporizador de posing</button></div>
...
<div class="tool-panel" id="panel-checklist" hidden><h3>ORDENA TU PRÓXIMA COMPETENCIA.</h3><p>Marca lo que ya resolviste. Esta lista evalúa organización, no determina si tu físico está listo para competir. Confirma los requisitos con tu federación y preparador.</p><form id="checkForm"><div id="checkItems"></div><button class="btn">Ver mis pendientes</button></form><div id="checkResult" class="tool-result" role="status" hidden></div></div>
...
async function checkAccess() {
  ...
  status.textContent = registered ? 'Acceso activo · Tus tres herramientas están desbloqueadas.' : ready ? 'Regístrate una vez. Recordaremos tu acceso en este navegador.' : 'Las herramientas están preparadas. El registro abrirá próximamente.';
}
await checkAccess();
...
document.querySelector('#accessForm').onsubmit = async e => {
  ...
  try {
    ...
    registered = true; status.textContent = 'Acceso activo · Tus tres herramientas están desbloqueadas.'; dialog.close(); e.target.reset(); const action = pending; pending = null; action?.();
...
const items = ['Confirmé federación, evento y división con mi preparador.', 'Revisé requisitos de inscripción, documentos y fecha límite.', 'Confirmé horarios de registro, pesaje y presentación.', 'Revisé vestuario y accesorios permitidos.', 'Tengo una lista de poses y transiciones que debo practicar.', 'Organicé música y formato de entrega, si aplica.', 'Resolví transporte, hospedaje y acompañamiento.', 'Preparé equipaje y contactos del organizador.', 'Acordé con mi preparador el seguimiento previo y posterior.'];
document.querySelector('#checkItems').innerHTML = items.map((item,i)=>`<label class="check-item"><input type="checkbox" value="${i}">${item}</label>`).join('');
document.querySelector('#checkForm').onsubmit = e => { e.preventDefault(); const missing = items.filter((_,i)=> !document.querySelector(`#checkItems input[value="${i}"]`).checked); gate(()=> {
  const out = document.querySelector('#checkResult'); out.hidden = false; out.replaceChildren();
  const h = document.createElement('h3'); h.textContent = `${items.length-missing.length} DE ${items.length} RESUELTOS`; out.append(h);
  const list = document.createElement('ul'); missing.forEach(text=>{ const li=document.createElement('li'); li.textContent=text; list.append(li); }); out.append(list);
  const p=document.createElement('p'); p.textContent=missing.length?'Estos son tus próximos pendientes.':'Organización completa según esta lista. Verifica los detalles con tu preparador.'; out.append(p); cta(out,'5 claves antes de competir');
}); };
